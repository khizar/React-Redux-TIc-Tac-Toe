...
const config = {
    title: 'Tic Tac Toe',                          // WebApp Title
    url: '',                                    // WebApp URL
...
// -----------------------------------------------------------------------------
tasks.set('clean', () => del(['public/dist/*', '!public/dist/*.css'], { dot: true }));
...
tasks.set('start', () => {
    ...
    return run('clean').then(() => new Promise(resolve => {
        ...
        compiler.plugin('done', stats => {
            ...
            const template = fs.readFileSync('./index.ejs', 'utf8');
            const render = ejs.compile(template, { filename: './index.ejs' });
            const output = render({ debug: true, bundle: `/dist/${bundle}`, css: '/dist/main.css', config });
            fs.writeFileSync('./public/index.html', output, 'utf8');
